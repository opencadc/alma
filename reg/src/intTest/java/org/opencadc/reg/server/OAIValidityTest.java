...
public class OAIValidityTest {
    ...
    public OAIValidityTest() {
        try {
            ...
            this.oaiEndpoint = registryClient.getServiceURL(URI.create("ivo://almascience.org/reg"),
                                                            Standards.REGISTRY_10,
                                                            AuthMethod.ANON, Standards.INTERFACE_REG_OAI);
    ...
    public void testListIdentifiersEmpty() {
        try {
            URL u = new URL(
                    oaiEndpoint.toExternalForm() + "?verb=ListIdentifiers&metadataPrefix=ivo_vor&until=1999-01-01");
            log.info(u.toExternalForm());
            ByteArrayOutputStream bos = new ByteArrayOutputStream();
            HttpGet get = new HttpGet(u, bos);
            get.run();
    ...
    public void testListIdentifiersFromUntil() {
        try {
            URL u = new URL(oaiEndpoint.toExternalForm() +
                            "?verb=ListIdentifiers&metadataPrefix=ivo_vor&from=2010-01-01T00:00:00Z&until=2018-12" +
                            "-31T11:59:59Z");
            log.info(u.toExternalForm());
            ByteArrayOutputStream bos = new ByteArrayOutputStream();
            HttpGet get = new HttpGet(u, bos);
    ...
    public void testGetRecord() {
        try {
            URL u = new URL(oaiEndpoint.toExternalForm() +
                            "?verb=GetRecord&metadataPrefix=ivo_vor&identifier=ivo://cadc.nrc.ca/tap");
            log.info(u.toExternalForm());
            ByteArrayOutputStream bos = new ByteArrayOutputStream();
            HttpGet get = new HttpGet(u, bos);
            get.run();
    ...
    public void testGetRecordsFrom() {
        try {
            URL u = new URL(oaiEndpoint.toExternalForm() +
                            "?verb=ListRecords&metadataPrefix=ivo_vor&from=2018-01-01T00:00:00Z");
            log.info(u.toExternalForm());
            ByteArrayOutputStream bos = new ByteArrayOutputStream();
            HttpGet get = new HttpGet(u, bos);
            get.run();
