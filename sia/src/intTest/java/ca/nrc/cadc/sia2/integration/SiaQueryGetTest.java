 ...
 * @author pdowler
 */
public class SiaQueryGetTest extends SyncUWSTest
{
    private static final Logger log = Logger.getLogger(SiaQueryGetTest.class);
    ...
    public SiaQueryGetTest()
    {
        ...
        // Redirects are not being followed, so disable POST.
    ...
    {
        Assert.assertEquals(String.format("%s failed.", result.name), 200, result.responseCode);
        Assert.assertEquals(String.format("%s failed.", result.name), "application/x-votable+xml",
                            result.contentType);

        try
        {
            Assert.assertNotNull(String.format("%s failed.", result.name), result.syncOutput);
            ...
            Assert.assertNotNull(String.format("%s QUERY_STATUS failed.", result.name), queryStatus);
            Assert.assertEquals(String.format("%s failed.", result.name), "OK", queryStatus);
        }
        catch(Exception ex)
        {
            log.error("unexpected exception", ex);
            throw new RuntimeException(String.format("%s failed.", result.name), ex);
        }
    }
}
