CREATE OR REPLACE FORCE VIEW ALMA.obscore (
    ...
    (SELECT DISTINCT aab.authors FROM ALMA.asa_bibliography aab JOIN ALMA.asa_project_bibliography aapb ON aab.bibcode = aapb.bibcode WHERE aapb.project_code = science.project_code AND ROWNUM < 2),
    (SELECT DISTINCT aab.abstract FROM ALMA.asa_bibliography aab JOIN ALMA.asa_project_bibliography aapb ON aab.bibcode = aapb.bibcode WHERE aapb.project_code = science.project_code AND ROWNUM < 2),
