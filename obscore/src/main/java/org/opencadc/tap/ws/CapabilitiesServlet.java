...
public class CapabilitiesServlet extends ca.nrc.cadc.vosi.CapabilitiesServlet {

    private static Logger log = Logger.getLogger(ca.nrc.cadc.vosi.CapabilitiesServlet.class);
    ...
    private void transformCapabilities(Document doc, HttpServletRequest request)
        ...
        List<Namespace> nsList = new ArrayList<>(root.getAdditionalNamespaces());
        ...
        for (Element e : accessURLs) {
            ...
            if (port > 0) {
                nurl = new URL(url.getProtocol(), hostname, port, url.getPath());
            } else {
                nurl = new URL(url.getProtocol(), hostname, url.getPath());
            }
