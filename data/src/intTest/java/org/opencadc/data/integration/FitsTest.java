...
public class FitsTest {
    ...
    public static void assertHDUEqual(final BasicHDU<?> expectedHDU, final BasicHDU<?> resultHDU) {
        ...
        final Object expectedData = expectedHDU.getData();
        final Object resultData = resultHDU.getData();

        Assert.assertEquals("Wrong data type.", ArrayFuncs.getBaseClass(expectedData),
                            ArrayFuncs.getBaseClass(resultData));
        Assert.assertArrayEquals("Wrong dimensions.", ArrayFuncs.getDimensions(expectedData),
                                 ArrayFuncs.getDimensions(resultData));
    ...
    public static void assertHeadersEqual(final Header expectedHeader, final Header resultHeader) {
        Arrays.stream(HEADER_CARD_KEYS_TO_CHECK).forEach(headerCardKey -> {
            ...
            if (expectedCard == null) {
                ...
                final String expectedValue = expectedCard.getValue();
                final String resultValue = resultCard.getValue();

                try {
                    Float.parseFloat(expectedValue);
                    Assert.assertEquals("Header " + headerCardKey.key() + " has the wrong value.",
                                        Float.parseFloat(expectedValue), Float.parseFloat(resultValue), 0.0F);
